...
public class OpenApiConfig {
        ...
        private static final String INSTANCE_EXAMPLE = "/api/v1/driver-location";
        ...
        public OpenAPI driverLocationOpenAPI(
                        @Value("${spring.application.name:driver-location-service}") String applicationName,
                        @Value("${server.servlet.context-path:}") String contextPath) {
                final String normalizedContextPath = normalizeContextPath(contextPath);
                return new OpenAPI()
                                .info(new Info()
                                                .title(applicationName + " API")
                                                .description("OpenAPI contract for the driver geo-location microservice.")
                                                .version("v1"))
                                .servers(List.of(new Server().url(normalizedContextPath).description("Base path")))
                                .externalDocs(new ExternalDocumentation()
                                                .description("Swagger UI")
                                                .url(swaggerUiUrl(normalizedContextPath)));
        }
        ...
        private Example validationExample() {
                ...
                payload.put(PROP_ERRORS, Map.of(
                                ...
                                "latitude", List.of("must be between -90 and 90")
                ));
        ...
        private Example businessExample() {
                ...
                payload.put(PROP_DETAIL, "Driver availability update rejected.");
                payload.put(PROP_INSTANCE, INSTANCE_EXAMPLE);
        ...
        private String swaggerUiUrl(String normalizedContextPath) {
                if ("/".equals(normalizedContextPath)) {
                        return "/swagger-ui.html";
                }
                return normalizedContextPath.endsWith("/")
                                ? normalizedContextPath + "swagger-ui.html"
                                : normalizedContextPath + "/swagger-ui.html";
        }
}
